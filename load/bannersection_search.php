...
$sections = fRecordSet::buildFromSQL(
	'BannerSection',
	"SELECT * FROM bannersection $where (name LIKE '%$query%' OR id_region IN (SELECT id_region FROM region WHERE name LIKE '%$query%')) LIMIT $start,$limit",
	"SELECT count(*) FROM bannersection $where (name LIKE '%$query%' OR id_region IN (SELECT id_region FROM region WHERE name LIKE '%$query%'))",
	$limit, // $limit
	$page  // $page
);
